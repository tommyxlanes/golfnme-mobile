...
export async function POST(req: NextRequest) {
  try {
    ...
    const token = await new SignJWT({
      sub: user.id, email: user.email, name: user.name, username: user.username,
    })
    ...
    return NextResponse.json({
      token,
      user: { id: user.id, email: user.email, name: user.name, username: user.username, avatarUrl: null, handicap: null },
    });
  } catch (error) {
